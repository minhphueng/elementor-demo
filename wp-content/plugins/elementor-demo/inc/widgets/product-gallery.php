...
class Product_Gallery extends Widget_Base {
    ...
    private function fetch_products( $limit = 9 ) {
        ...
        if ( false === $products ) {
            ...
            $response = wp_remote_get( 'https://dummyjson.com/products?limit=' . $limit, $args );
    ...
    private function fetch_collections() {
        ...
        if ( false === $categories ) {
            ...
            $response = wp_remote_get( 'https://dummyjson.com/products/categories', $args );
            if ( ! is_wp_error( $response ) ) {
                $body = wp_remote_retrieve_body( $response );
                $categories = json_decode( $body, true );
                if ( is_array( $categories ) ) {
                    set_transient( $cat_transient_key, $categories, 30 * MINUTE_IN_SECONDS );
                } else {
                    $categories = array();
                }
            } else {
